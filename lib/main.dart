...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  final List<Task> _tasks = [
    ...
    Task(
      id: 't4',
      title: 'Deep Work: Flutter Project',
      duration: 90,
      energy: 'High',
    ),
  ...
  int _points = 100;
  int _streak = 0;
  double get _multiplier => _streak >= 5
      ? 2.0
      : _streak >= 3
      ? 1.5
      : 1.0;
  ...
  void _handleCheckTap(Task task) {
    if (_completingIds.contains(task.id)) return;
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          duration: const Duration(milliseconds: 900),
          content: Text(
            '✅ ${task.title} complete! +$earned pts',
  ...
  void _addCustomTask() {
    ...
    if (title.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          backgroundColor: AppColors.synthPink,
  ...
  void _toggleEquip(ShopItem item) {
    if (item.category == ItemCategory.aura)
      _equippedAura = (_equippedAura == item.id) ? null : item.id;
    if (item.category == ItemCategory.eyewear)
      _equippedEyewear = (_equippedEyewear == item.id) ? null : item.id;
    if (item.category == ItemCategory.headwear)
      _equippedHeadwear = (_equippedHeadwear == item.id) ? null : item.id;
  }

  bool _isEquipped(ShopItem item) {
    if (item.category == ItemCategory.aura) return _equippedAura == item.id;
    if (item.category == ItemCategory.eyewear)
      return _equippedEyewear == item.id;
    if (item.category == ItemCategory.headwear)
      return _equippedHeadwear == item.id;
    return false;
  }
  ...
  Widget build(BuildContext context) {
    final List<Task> filtered = _tasks
        .where(
          (t) =>
              (!t.completed || _completingIds.contains(t.id)) &&
              t.duration <= _timePool &&
              t.energy == _selectedEnergy,
        )
        .toList();
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            ...
            SliverToBoxAdapter(
              child: _buildSectionTitle('🎯 MATCHED TASKS', filtered.length),
            ),
            filtered.isEmpty
                ? SliverToBoxAdapter(child: _buildEmptyState())
                : SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (ctx, i) => _buildTaskCard(filtered[i], i),
  ...
  Widget _buildStreakBadge() {
    ...
    Widget badge = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: isHot
            ? [
                BoxShadow(
                  color: AppColors.synthPink.withValues(alpha: 0.55),
                  blurRadius: 14,
                  spreadRadius: 1,
                ),
              ]
      ...
      child: Row(
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              color: isHot ? AppColors.synthPink : AppColors.textMuted,
              fontWeight: FontWeight.bold,
              fontSize: 13,
            ),
          ),
    ...
    badge = AnimatedBuilder(
      ...
      builder: (ctx, child) => Transform.scale(
        scale: isHot ? 1.0 + _pulseController.value * 0.08 : 1.0,
        child: child,
      ),
      child: badge,
    );
  ...
  Widget _buildCoinBank() {
    final Widget bank = Container(
  ...
  Widget _buildAvatarSection() {
    final String key =
        'av-$_avatarGender-$_equippedAura-$_equippedEyewear-$_equippedHeadwear';
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.neonOrange.withValues(alpha: 0.25)),
      ),
      child: Column(
        children: [
          TweenAnimationBuilder<double>(
            ...
            child: CustomPaint(
              ...
              painter: AvatarPainter(
                ...
                auraId: _equippedAura,
                eyewearId: _equippedEyewear,
                headwearId: _equippedHeadwear,
              ),
            ),
          ),
          ...
          Row(
            children: ['Female', 'Male'].map((g) {
              ...
              return Expanded(
                child: TapScale(
                  onTap: () {
                    if (_avatarGender != g) setState(() => _avatarGender = g);
                  },
      ...
      children: [
        Row(
          ...
          children: [
            ...
            AnimatedSwitcher(
              ...
              child: Text(
                '${_timePool.round()} min',
                key: ValueKey<int>(_timePool.round()),
                style: const TextStyle(
                  color: AppColors.neonOrange,
                  fontWeight: FontWeight.bold,
                  fontSize: 15,
        ...
        SliderTheme(
          data: SliderTheme.of(context).copyWith(
            ...
            valueIndicatorColor: AppColors.neonOrange,
          ),
          child: Slider(
            min: 5,
            max: 120,
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: sel
                        ? [
                            BoxShadow(
                              color: AppColors.synthPink.withValues(
                                alpha: 0.45,
                              ),
                              blurRadius: 10,
                              spreadRadius: 0.5,
  ...
  Widget _buildEmptyState() => TweenAnimationBuilder<double>(
    key: ValueKey('empty-$_selectedEnergy-${_timePool.round()}'),
    ...
    child: Container(
      ...
      child: const Center(
        child: Text(
          'No tasks match your current vibe.\nAdjust your energy or time pool 🔍',
          textAlign: TextAlign.center,
  ...
  Widget _buildTaskCard(Task task, int index) {
    final Color ec = task.energy == 'Low'
        ? Colors.greenAccent
        : task.energy == 'Medium'
        ? AppColors.neonOrange
        : AppColors.synthPink;
    final bool isCompleting = _completingIds.contains(task.id);
    final bool isDeleting = _deletingIds.contains(task.id);
    final bool fading = isCompleting || isDeleting;

    return EntranceFade(
      key: ValueKey(task.id),
      delay: Duration(milliseconds: (index * 40).clamp(0, 280)),
      child: AnimatedScale(
        scale: fading ? 0.85 : 1.0,
        ...
        child: AnimatedOpacity(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: AppColors.surface,
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: ec.withValues(alpha: 0.35)),
            ),
            child: Row(
              children: [
                TapScale(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Icon(
                      Icons.check,
                      size: 16,
                      color: isCompleting ? Colors.black : AppColors.neonOrange,
                    ),
                  ),
                ),
                ...
                TapScale(
                  onTap: () => _handleDeleteTap(task.id),
                  child: const Padding(
                    padding: EdgeInsets.all(8),
                    child: Icon(
                      Icons.delete_outline,
                      color: AppColors.synthPink,
                      size: 22,
                    ),
                  ),
                ),
  ...
  Widget _buildCustomTaskGenerator() => Container(
    ...
    decoration: BoxDecoration(
      color: AppColors.surface,
      borderRadius: BorderRadius.circular(18),
      border: Border.all(color: AppColors.synthPink.withValues(alpha: 0.3)),
    ),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText: 'e.g. Write journal entry',
        ...
        Row(
          children: [
            Expanded(child: _buildEnergyDropdown()),
            const SizedBox(width: 10),
  ...
  Widget _buildEnergyDropdown() => Container(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<String>(
        value: _newTaskEnergy,
        isExpanded: true,
        dropdownColor: AppColors.surface,
        iconEnabledColor: AppColors.neonOrange,
        style: const TextStyle(color: AppColors.textPrimary, fontSize: 13),
        items: _energyLevels
            .map((e) => DropdownMenuItem<String>(value: e, child: Text(e)))
            .toList(),
        onChanged: (v) {
          if (v != null) setState(() => _newTaskEnergy = v);
        },
      ),
    ),
  );
  ...
  Widget _buildDurationDropdown() => Container(
    ...
    child: DropdownButtonHideUnderline(
      child: DropdownButton<int>(
        value: _newTaskDuration,
        isExpanded: true,
        dropdownColor: AppColors.surface,
        iconEnabledColor: AppColors.neonOrange,
        style: const TextStyle(color: AppColors.textPrimary, fontSize: 13),
        items: _durationMilestones
            .map((d) => DropdownMenuItem<int>(value: d, child: Text('$d min')))
            .toList(),
        onChanged: (v) {
          if (v != null) setState(() => _newTaskDuration = v);
        },
      ),
    ),
  );
  ...
  Widget _buildShopSection() => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        const Text(
          'Unlock cosmetics with your points',
          style: TextStyle(color: AppColors.textMuted, fontSize: 12),
        ),
        const SizedBox(height: 12),
        _shopLabel('AURA'),
        ..._shopItems
            .where((i) => i.category == ItemCategory.aura)
            .map(_shopRow),
        const SizedBox(height: 8),
        _shopLabel('EYEWEAR'),
        ..._shopItems
            .where((i) => i.category == ItemCategory.eyewear)
            .map(_shopRow),
        const SizedBox(height: 8),
        _shopLabel('HEADWEAR'),
        ..._shopItems
            .where((i) => i.category == ItemCategory.headwear)
            .map(_shopRow),
      ],
    ),
  );
  ...
  Widget _shopRow(ShopItem item) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: equipped ? AppColors.neonOrange : Colors.transparent,
          width: 1.5,
        ),
        boxShadow: equipped
            ? [
      ...
      child: Row(
        children: [
          TweenAnimationBuilder<double>(
            key: ValueKey('si-${item.id}-${item.unlocked}-$equipped'),
          ...
          TapScale(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: !item.unlocked
                    ? AppColors.neonOrange
                    : (equipped ? AppColors.synthPink : AppColors.surface),
                borderRadius: BorderRadius.circular(10),
              ),
              child: Text(
                !item.unlocked ? 'BUY' : (equipped ? 'UNEQUIP' : 'EQUIP'),
                style: TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 11,
                  color: !item.unlocked ? Colors.black : Colors.white,
                ),
              ),
            ),
          ),
...
class AvatarPainter extends CustomPainter {
        ...
        Paint()
          ..shader = RadialGradient(
            colors: [
              ...
              AppColors.neonOrange.withValues(alpha: 0),
            ],
          ).createShader(Rect.fromCircle(center: c, radius: 93)),
      );
      final Paint fl = Paint()
        ..color = AppColors.synthPink.withValues(alpha: 0.38);
      for (int i = 0; i < 7; i++) {
  ...
  void _drawHairBack(Canvas canvas) {
    final Paint hp = Paint()
      ..color = gender == 'Female'
          ? AppColors.synthPink
          : const Color(0xFF3FD3C6);
    if (gender == 'Female') {
      canvas.drawRRect(
        RRect.fromRectAndCorners(
          const Rect.fromLTRB(28, 30, 162, 155),
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          const Rect.fromLTRB(34, 34, 156, 100),
  ...
  void _drawHead(Canvas canvas) {
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTRB(headL, headTop, headR, headB),
  ...
  void _drawHairFront(Canvas canvas) {
    ...
    if (gender == 'Female') {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          const Rect.fromLTRB(44, 44, 118, 68),
  ...
  void _drawCheeks(Canvas canvas) {
    final Paint blush = Paint()
      ..color = AppColors.synthPink.withValues(alpha: 0.2);
  ...
  void _drawEyebrows(Canvas canvas) {
    final Paint brow = Paint()
      ...
      ..style = PaintingStyle.stroke
      ..strokeWidth = gender == 'Female' ? 2.2 : 3.2
      ..strokeCap = StrokeCap.round;
    if (gender == 'Female') {
      // Arched brows
      final Path l = Path()
        ..moveTo(60, 74)
        ..quadraticBezierTo(72, 68, 84, 72);
      final Path r = Path()
        ..moveTo(106, 72)
        ..quadraticBezierTo(118, 68, 130, 74);
      canvas.drawPath(l, brow);
      canvas.drawPath(r, brow);
    } else {
      // Straight flat brows
  ...
  void _drawEyes(Canvas canvas) {
    ...
    final Paint iris = Paint()
      ..color = gender == 'Female'
          ? AppColors.synthPink.withValues(alpha: 0.8)
          : Colors.cyanAccent.withValues(alpha: 0.8);
    final Paint shine = Paint()..color = Colors.white;

    for (final double x in [leftEyeX, rightEyeX]) {
      canvas.drawOval(
        Rect.fromCenter(center: Offset(x, eyeY), width: 26, height: 22),
        white,
      );
  ...
  void _drawMouth(Canvas canvas) {
    canvas.drawPath(
      Path()
        ..moveTo(cx - 14, 122)
        ..quadraticBezierTo(cx, gender == 'Female' ? 134 : 130, cx + 14, 122),
      Paint()
    ...
    if (gender == 'Female') {
      ...
      canvas.drawPath(
        Path()
          ..moveTo(cx - 9, 122)
          ..quadraticBezierTo(cx, 130, cx + 9, 122),
        Paint()
          ..color = AppColors.synthPink.withValues(alpha: 0.35)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 5
          ..strokeCap = StrokeCap.round,
      );
  ...
  // EYEWEAR  -- anchored exactly to (leftEyeX,eyeY) and (rightEyeX,eyeY)
  void _drawEyewear(Canvas canvas) {
    if (eyewearId == null) return;
    if (eyewearId == 'e1') {
      // Cyber shades: dark rectangular lenses + thin cyan rims + arms
      ...
      for (final double x in [leftEyeX, rightEyeX]) {
        final RRect r = RRect.fromRectAndRadius(
          Rect.fromCenter(center: Offset(x, eyeY), width: 34, height: 22),
      ...
      // Bridge
      canvas.drawLine(
        Offset(leftEyeX + 17, eyeY),
        Offset(rightEyeX - 17, eyeY),
        arm,
      );
      // Arms to ears
      ...
      // Disguise: full-face band with eye holes cut out
      final Path band = Path()
        ..addRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTRB(headL + 4, eyeY - 16, headR - 4, eyeY + 16),
  ...
  void _drawHeadwear(Canvas canvas) {
    ...
    if (headwearId == 'h1') {
      ...
      canvas.drawCircle(
        const Offset(82, 16),
        4,
        Paint()..color = AppColors.synthPink,
      );
      canvas.drawCircle(
        const Offset(cx, 30),
        4,
      ...
      canvas.drawRRect(
        RRect.fromRectAndCorners(
          const Rect.fromLTRB(66, 8, 124, 40),
